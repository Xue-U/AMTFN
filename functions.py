...
def Evaluation_index(output_list,pred,test_y_list):
    ...
    recall_score_test = metrics.recall_score(test_y_l, y_yuce, pos_label=1) #以短期幸存者作为positive
    acc_score_test = metrics.accuracy_score(test_y_l, y_yuce)
    precision_test = metrics.precision_score(test_y_l, y_yuce, pos_label=1)
    F1_score_test = metrics.f1_score(test_y_l, y_yuce, pos_label=1)
    test_auc = metrics.roc_auc_score(test_y_l, output_list)
    fpr, tpr, thersholds = metrics.roc_curve(test_y_l, output_list)
    mcc,sn=matthews_correlation(test_y_l,y_yuce)
    return acc_score_test,recall_score_test,precision_test,F1_score_test,test_auc,mcc,sn,fpr,tpr
